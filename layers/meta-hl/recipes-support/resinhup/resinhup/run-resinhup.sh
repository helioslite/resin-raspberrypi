...
if [ -f /mnt/boot/device-type.json ]; then
    # get slug from devicetype.json (see resinhup docker image):
    slug=$(jq -r .slug /mnt/boot/device-type.json)
elif [ -f /mnt/boot/config.json ]; then
    ...
    log ERROR "Don't know where device-type.json/config.json is."
fi
if [ -z $slug ]; then
...
# HL: update-resin-supervisor.timer is not present on resinos-hl
...
if [ ! -z "$UPDATER_SUPERVISOR_TAG" ]; then
    ...
    if [ -z "$SUPERVISOR_REGISTRY" ]; then
        ...
        UPDATER_SUPERVISOR_IMAGE="$SUPERVISOR_REGISTRY/$DOCKER_REPO/$arch-supervisor"
    fi
...
#$DOCKER pull registry.resinstaging.io/resinhup/$slug-resinhup:$TAG
$DOCKER pull $DOCKER_REPO/$slug-resinhup:$TAG
if [ $? -ne 0 ]; then
    tryup
    log ERROR "Could not pull $DOCKER_REPO/$slug-resinhup:$TAG ."
fi
...
$DOCKER run --privileged --rm --net=host $RESINHUP_ENV \
    ...
    $DOCKER_REPO/$slug-resinhup:$TAG
    #registry.resinstaging.io/resinhup/$slug-resinhup:$TAG
RESINHUP_EXIT=$?
# RESINHUP_EXIT
#   0 - update done
